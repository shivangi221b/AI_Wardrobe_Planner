...
# Required garment nouns for the noun-gate pass (stricter than hint boosting).
_GARMENT_NOUNS: frozenset[str] = frozenset(
    {
        "shirt",
        "shirts",
        "tshirt",
...
def _contains_garment_noun(blob_lower: str) -> bool:
    return any(_token_appears_in_lower_text(noun, blob_lower) for noun in _GARMENT_NOUNS)
...
def filter_and_rank_image_results(
    ...
    *,
    ...
    kept = [
        ...
        if s > strong_junk_threshold and _contains_garment_noun(_result_text_blob(it))
    ]

    if not kept:
        # If score threshold is too strict, still enforce garment noun-gate.
        kept = [it for _, it in scored if _contains_garment_noun(_result_text_blob(it))]

    if not kept:
        # Last-resort fallback to avoid returning an empty list on sparse/noisy metadata.
